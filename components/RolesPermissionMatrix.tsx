...
export default function RolesPermissionMatrix() {
    ...
    try {
      ...
      const [rolesResponse, permissionsResponse, matrixResponse] =
        await Promise.all([
          supabase
            .from("roles")
            .select("code, name, description, sort_order, is_system")
      ...
      setPermissions((permissionsResponse.data || []) as PermissionRow[]);
      setRolePermissions(
  ...
  const permissionMap = useMemo(() => {
    ...
    rolePermissions
      ...
      .forEach((entry) => {
        map.add(`${entry.role_code}:${entry.permission_code}`);
      });
      ...
      new Map(
        roles.map((role) => [
          ...
          permissions.filter((permission) =>
            permissionMap.has(`${role.code}:${permission.code}`)
          ).length,
        ])
      ),
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300 sm:text-base">
              Review exactly which VertexERP modules and actions are available
              to each built-in company role.
            </p>
          </div>
          ...
          <button
            ...
            onClick={loadMatrix}
            disabled={isLoading}
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-900 px-5 py-3 font-bold text-white hover:bg-slate-800 disabled:opacity-50"
          >
            <RefreshCw
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="Modules" value={groupedPermissions.length} />
        <Metric
          label="Your Role"
      ...
      >
        <div className="flex items-start gap-3">
          {canManageRoles ? (
            ...
            <Eye className="mt-0.5 h-6 w-6 text-blue-700" />
          )}
          <div>
            <h2
              className={`font-black ${
                canManageRoles ? "text-violet-950" : "text-blue-950"
              }`}
            >
              {canManageRoles
            ...
            <p
              className={`mt-1 text-sm leading-6 ${
                canManageRoles ? "text-violet-800" : "text-blue-800"
              }`}
            >
              Built-in role permissions are currently controlled by the
              database migration. Custom role editing will be added after
              business-table RLS enforcement is complete.
            </p>
          </div>
        </div>
      </section>
          ...
          >
            ...
            <div className="mt-5 space-y-4">
              {groupedPermissions.map(([module, modulePermissions]) => {
                const allowed = modulePermissions.filter((permission) =>
                  permissionMap.has(`${role.code}:${permission.code}`)
                );
      ...
      <section className="hidden overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl md:block">
        <div className="overflow-x-auto">
          <table className="min-w-[1250px] w-full">
            <thead className="sticky top-0 bg-slate-950 text-white">
              <tr>
                <th className="sticky left-0 z-20 min-w-[300px] bg-slate-950 px-5 py-4 text-left text-sm font-black">
                  Permission
                </th>
                {roles.map((role) => (
                  <th
            ...
            <tbody>
              {isLoading ? (
                ...
                groupedPermissions.flatMap(([module, modulePermissions]) => [
                  <tr key={`${module}-heading`} className="bg-slate-100">
                    <td
                      colSpan={roles.length + 1}
                      className="px-5 py-3 text-xs font-black uppercase tracking-[0.16em] text-slate-600"
                    >
                      {module}
                    </td>
                  </tr>,
      ...
      {!isLoading && roles.length === 0 && (
        <section className="rounded-3xl border border-amber-200 bg-amber-50 p-6 text-center">
          ...
          <p className="mt-1 text-sm text-amber-800">
            Confirm that the Roles &amp; Permissions foundation SQL completed
            successfully.
          </p>
        </section>
      )}

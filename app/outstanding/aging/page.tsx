...
import OutstandingAgingManager from "@/components/OutstandingAgingManager";
import { usePermissions } from "@/hooks/usePermissions";

export default function OutstandingAgingPage() {
  const {
    access,
    can,
          ...
          {isPermissionLoading ? (
            ...
            <section className="rounded-3xl border border-red-200 bg-white p-8 text-center shadow-xl sm:p-12">
              ...
              <h1 className="mt-4 text-2xl font-black text-slate-900">
                Outstanding Access Restricted
              </h1>
              <p className="mx-auto mt-3 max-w-2xl leading-7 text-slate-600">
                Your current role does not include the outstanding.view
                permission for the active company.
              </p>
            </section>
          ) : (
            <OutstandingAgingManager />
          )}
        </main>
      </div>
    </div>
